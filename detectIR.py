...
def process_images(path, model):
    ...
    for   i in tqdm(range(len(all_file))):
        ...
        for pos, cls_value in zip(xywh_, cls_):
            ...
            cv2.rectangle(maskrgb, tuple(pt1), tuple(pt2), color, 4)
            # 限制一下标签位置
            xfill=20
            yfill=15
            text_x=pt1[0]
            ...
            cv2.imwrite("./detect2/ir_"+files,maskrgb)
...
def main():
    ...
    if os.path.exists('./detect2') !=True:
        os.makedirs('./detect2') 

    model = load_model("/home/mjy/ultralytics3/runs/detect/IR/weights/best.pt", device)
